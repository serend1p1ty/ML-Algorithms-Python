...
class DecisionTree:
    ...
    Attributes:
        attribute: 当前节点要根据什么属性划分集合
        threshold: 用来划分集合的属性为连续属性时，划分集合所使用的阈值
        result: 当前节点为叶子节点时，预测的结果
        children: 子节点
    """
    ...
    def devide_by_attribute(self, X, y, attribute):
        ...
        if isinstance(X[0][attribute], str):
            ...
            column_copy = sorted(column.copy())
            ...
            base_entro = entropy(y)
            max_gain = -1

            # 选择最大化信息增益的阈值
            for candidate in candidates:

                def split_func(i, c=candidate):
                    return column[i] <= c
                ...
                entro_sum = p * entropy(index(y, part1)) + (1 - p) * entropy(index(y, part2))
    ...
    def max_info_gain(self, X, y, attributes):
        ...
        for attrID in attributes:
            ...
            for key in subsets:
                ...
                entro_sum += p * entropy(index(y, subsets[key]))
            if base_entro - entro_sum > max_gain:
                max_gain = base_entro - entro_sum
                best_attr = attrID
